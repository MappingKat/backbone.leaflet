  ...
  hooks: {
    init: function(callback) {
      document.getElementById('my-custom-module').innerHTML = '' +
        '<form id="form-search" href="#" style="border-bottom: solid 1px #EDEBE4; height: 55px;">' +
        ...
        '<table class="table table-bordered table-condensed" id="unit-table">' +
          '<thead>' +
            ...
            '</tr>' +
          '</thead>' +
          '<tbody id="unit-table-body" class="list">' +
          '</tbody>' +
        '</table>';
      ...
      $(document).ready(function(){
        $.getJSON('../battles.json', function(data) {
          ...
          var appending = $('#unit-table-body').html(html);
          console.log(appending);

          $('.btn-primary.search').on('click', function(){
            var searchQuery = $('#input-search').val();
            if ($('#unit-table-body').empty()) {
          ...
          // values to append will be in this array
          // loops thru json and push the resuts that we want into vales array

          });
        });


// grab lat and long and fit bounds to marker
